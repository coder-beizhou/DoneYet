...
- **设置** — 8 套主题风格、不透明度滑块、关闭按钮行为(退出/最小化到托盘)、开机自启、固定到桌面底层、中英文切换
- **多语言** — 中英文实时切换(设置→语言),品牌名随语言变(办了么/DoneYet),日历 24 节气 + 7 节日双语,托盘菜单/窗口标题/提醒通知跟随语言
- **作者署名 + 彩蛋** — 花体 BEIZHOU 署名藏于各角落(Settings 页脚/状态栏/便签/日历标题栏);连点 5 次或 Konami 码(↑↑↓↓←→←→BA)解锁彩纸 + 彩虹闪烁彩蛋
...
- **exe 安装版**: `办了么_1.1.0_x64-setup.exe`（NSIS 安装包）
